...
function sh:init()
	...
	self.rainTexture = { }
	for i = 1, 5 do
		self.rainTexture[i] = love.graphics.newImage(root .. "rain_" .. i .. ".png")
		self.rainTexture[i]:setWrap("repeat")
	end
	
	--splash textures
	self.splashTexture = love.graphics.newImage(root .. "splash.png")
	self.wetnessTexture = love.graphics.newImage(root .. "wetness.png")
	
	self.wetnessTexture:setWrap("repeat")
...
function sh:constructDefines(dream)
	return [[
		...
		extern Image wetnessTexture;
		extern float wetness;
	]]
end

function sh:constructPixel(mat)
	return [[
		vec3 rainNormal = normalize(Texel(splashes, VertexPos.xz).xyz);
		
		float rainNoise = Texel(wetnessTexture, VertexPos.xz * 0.17).r;
...
function sh:perShader(shaderObject)
	...
	shader:send("wetnessTexture", sh.wetnessTexture)
...
function sh:render(cam, canvases, scene)
	...
	local t = self.rainTexture[self.strength]
	...
	love.graphics.draw(self.object_rain.objects.Plane:getMesh())
	...
	if sh.rain > 0 or lastRender then
		...
		if sh.rain > 0 then
			...
			for d,s in ipairs(sh.drops) do
				...
				love.graphics.draw(sh.splashTexture, s.x * w, s.y * h, 0, s.size * (1.0 + s.time), nil, 32, 32)
				love.graphics.draw(sh.splashTexture, s.x * w, wy * h, 0, s.size * (1.0 + s.time), nil, 32, 32)
				love.graphics.draw(sh.splashTexture, wx * w, s.y * h, 0, s.size * (1.0 + s.time), nil, 32, 32)
				love.graphics.draw(sh.splashTexture, wx * w, wy * h, 0, s.size * (1.0 + s.time), nil, 32, 32)
			end
		end
		love.graphics.pop()
	end
